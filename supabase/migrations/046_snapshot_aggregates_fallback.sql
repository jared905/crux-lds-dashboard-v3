-- 046: Update snapshot aggregates to fall back to videos table for analytics data
-- The Analytics API writes impressions/CTR/retention/subscribers to the videos table,
-- but the snapshot-based date filtering only read from video_snapshots — which often
-- doesn't have this data. This update adds COALESCE fallbacks to the videos table.
...
AS $$
  SELECT
    ...
    -- Impressions: prefer snapshot sum, fall back to videos table (from Reporting API sync)
    COALESCE(NULLIF(SUM(vs.impressions), 0), v.impressions) AS impressions,
    -- CTR: prefer snapshot weighted avg, fall back to videos table (from Reporting API sync)
    CASE
      WHEN SUM(vs.impressions) > 0
      THEN SUM(vs.ctr * vs.impressions) / SUM(vs.impressions)
      ELSE v.ctr
    END AS ctr,
    -- Retention: prefer snapshot weighted avg, fall back to videos table
    CASE
