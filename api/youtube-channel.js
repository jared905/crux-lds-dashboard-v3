...
// --- Encryption helpers for OAuth token handling (lifetime stats) ---
...
/**
 * Handle lifetime stats request: fetches channel-level lifetime watch hours
 * from YouTube Analytics API via OAuth connections.
 */
async function handleLifetimeStats(req, res) {
    ...
    try {
      ...
      if (data.rows && data.rows.length > 0) {
        ...
        perChannel[connection.youtube_channel_title || connection.youtube_channel_id] = {
          watchHours, views, subscribersGained: subsGained,
        };
  ...
  return res.status(200).json({ totalWatchHours, perChannel });
}
...
export default async function handler(req, res) {
  ...
  // Route to lifetime stats handler if action=lifetime
  const { action } = req.body || {};
  if (action === 'lifetime') {
    return handleLifetimeStats(req, res);
  }
