...
import { Check, ChevronDown, ChevronUp, ExternalLink, FileText, Lightbulb } from 'lucide-react';

const fmtInt = (n) => (!n || isNaN(n)) ? "0" : Math.round(n).toLocaleString();

const SOURCE_STYLES = {
  diagnostic: { bg: "rgba(96, 165, 250, 0.1)", border: "var(--accent-text)", text: "var(--accent-text)", label: "Diagnostics" },
  gap:        { bg: "rgba(168, 85, 247, 0.1)", border: "var(--accent-text)", text: "var(--accent-text)", label: "Competitor Gap" },
  outlier:    { bg: "rgba(251, 191, 36, 0.1)", border: "var(--warn-text)", text: "var(--warn-text)", label: "Breakout Intel" },
};

const IMPACT_STYLES = {
  high:   { color: "var(--pos)", label: "High Impact" },
  medium: { color: "var(--warn-text)", label: "Med Impact" },
  low:    { color: "var(--faint)", label: "Low Impact" },
};

const EFFORT_MAP = {
  Low: { color: "var(--pos)", label: "Low Effort" },
  low: { color: "var(--pos)", label: "Low Effort" },
  Medium: { color: "var(--warn-text)", label: "Med Effort" },
  medium: { color: "var(--warn-text)", label: "Med Effort" },
  High: { color: "var(--neg)", label: "High Effort" },
  high: { color: "var(--neg)", label: "High Effort" },
};
...
export default function NextUpPanel({ rows, activeClient }) {
  ...
  return (
    <div className="section-card" style={{
      background: "linear-gradient(135deg, rgba(205, 242, 0, 0.05), rgba(96, 165, 250, 0.03))",
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <div style={{
            ...
            <Lightbulb size={22} style={{ color: "var(--pos)" }} />
          </div>
          <div>
            <div style={{ fontSize: "26px", fontWeight: "700", color: "var(--ink)" }}>What to Create Next</div>
            <div style={{ fontSize: "13px", color: "var(--muted)", marginTop: "2px" }}>
              Prioritized actions based on {recommendations.length} signals — turn insights into content
              {totalOpportunity > 0 && (
                <span style={{ color: "var(--pos)", fontWeight: "600", marginLeft: "8px" }}>
        ...
        {[
          ...
          <button
            ...
            style={{
              padding: "6px 14px",
              borderRadius: "20px",
              border: filter === f.key ? "2px solid var(--pos)" : "1px solid var(--border)",
              backgroundColor: filter === f.key ? "rgba(205, 242, 0, 0.15)" : "var(--input-bg)",
              color: filter === f.key ? "var(--pos)" : "var(--text)",
              fontSize: "12px",
              fontWeight: "700",
              cursor: "pointer",
            ...
            <div key={rec.id} style={{
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap", marginBottom: "6px" }}>
                    ...
                    <span style={{
                      ...
                      color: impactStyle.color, background: `color-mix(in srgb, ${impactStyle.color} 8%, transparent)`,
                    ...
                    <span style={{
                      fontSize: "10px", fontWeight: "700",
                      color: effortStyle.color, background: `color-mix(in srgb, ${effortStyle.color} 8%, transparent)`,
                  ...
                  {rec.action && (
                    <div style={{
                      fontSize: "13px", color: "var(--pos)", fontWeight: "600",
                ...
                <div style={{ display: "flex", gap: "6px", flexShrink: 0 }}>
                  ...
                  <button
                    ...
                    style={{
                      padding: "6px 12px",
                      borderRadius: "6px",
                      border: sentToBrief[rec.id] ? "1px solid var(--pos)" : "1px solid var(--border)",
                      backgroundColor: sentToBrief[rec.id] ? "rgba(205, 242, 0, 0.15)" : "var(--input-bg)",
                      color: sentToBrief[rec.id] ? "var(--pos)" : "var(--text)",
                      fontSize: "11px",
                      fontWeight: "700",
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: "1px solid var(--border)", backgroundColor: "var(--input-bg)",
              ...
              {isExpanded && (
                <div style={{ marginTop: "12px", paddingTop: "12px", borderTop: "1px solid var(--border)" }}>
                  {rec.description && (
                    <div style={{ fontSize: "13px", color: "var(--text)", marginBottom: "10px", lineHeight: "1.5", whiteSpace: "pre-line" }}>
                      {rec.description}
                    </div>
                  )}
                  ...
                  {rec.evidence && (
                    <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", marginBottom: "10px" }}>
                      {rec.evidence.competitorStat && (
                        <div style={{ fontSize: "12px", color: "var(--accent-text)", background: "rgba(168, 85, 247, 0.08)", padding: "4px 10px", borderRadius: "6px" }}>
                          Competitors: {rec.evidence.competitorStat}
                        </div>
                      )}
                      {rec.evidence.clientStat && (
                        <div style={{ fontSize: "12px", color: "var(--accent-text)", background: "rgba(96, 165, 250, 0.08)", padding: "4px 10px", borderRadius: "6px" }}>
                          You: {rec.evidence.clientStat}
                        </div>
                      )}
                      {rec.evidence.outlierScore && (
                        <div style={{ fontSize: "12px", color: "var(--warn)", background: "rgba(251, 191, 36, 0.08)", padding: "4px 10px", borderRadius: "6px" }}>
                          {rec.evidence.outlierScore.toFixed(1)}x channel average ({fmtInt(rec.evidence.views)} views)
                        </div>
                      )}
                      {rec.evidence.delta && (
                        <div style={{ fontSize: "12px", color: "var(--muted)", background: "rgba(158, 158, 158, 0.08)", padding: "4px 10px", borderRadius: "6px" }}>
                          {rec.evidence.delta}
                        </div>
                      )}
                    </div>
                  ...
                  {rec.videoExamples?.length > 0 && (
                    <div>
                      <div style={{ fontSize: "11px", fontWeight: "600", color: "var(--outline)", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "8px" }}>
                        Reference Videos
                      </div>
                        ...
                        {rec.videoExamples.slice(0, 5).map((v, i) => (
                          <div key={i} style={{
                            ...
                            {v.youtubeVideoId ? (
                              ...
                              <div style={{ width: "64px", height: "36px", borderRadius: "4px", backgroundColor: "var(--input-bg)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                                <FileText size={12} style={{ color: "var(--outline-variant)" }} />
                              </div>
                            )}
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              {v.channel && <div style={{ fontSize: "10px", color: "var(--faint)", marginTop: "1px" }}>{v.channel}</div>}
                            </div>
                            <div style={{ fontSize: "12px", color: "var(--pos)", fontWeight: "600", flexShrink: 0 }}>
