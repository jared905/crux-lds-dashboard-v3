...
import { Activity, AlertCircle, CheckCircle2, DollarSign, ExternalLink, Key } from 'lucide-react';
import YouTubeOAuthInvitesSection from './YouTubeOAuthInvitesSection.jsx';
import YouTubeOAuthSettings from './YouTubeOAuthSettings.jsx';
    ...
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <Key size={24} style={{ color: "var(--accent-text)" }} />
          <div>
            <h2 style={{ fontSize: "20px", fontWeight: "700", margin: 0 }}>API Settings</h2>
            <p style={{ fontSize: "13px", color: "var(--muted)", margin: "4px 0 0" }}>Configure your API keys and usage limits</p>
          </div>
        </div>
        {saved && (
          <div style={{
            display: "flex", alignItems: "center", gap: "8px",
            padding: "8px 16px", background: "rgba(205, 242, 0, 0.1)",
            border: "1px solid rgba(205, 242, 0, 0.3)", borderRadius: "8px",
            color: "var(--pos)", fontSize: "13px", fontWeight: "600",
          }}>
      ...
      <div style={cardStyle}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "16px" }}>
          <Key size={18} style={{ color: "var(--accent-text)" }} />
          <h3 style={{ fontSize: "16px", fontWeight: "600", margin: 0 }}>Claude API</h3>
        </div>

        <div style={{ marginBottom: "16px" }}>
          <label style={{ display: "block", fontSize: "13px", color: "var(--muted)", marginBottom: "8px" }}>API Key</label>
          ...
          <a
            ...
            style={{ display: "inline-flex", alignItems: "center", gap: "4px", fontSize: "12px", color: "var(--accent-text)", marginTop: "8px", textDecoration: "none" }}
          >
        ...
        <div>
          <label style={{ display: "block", fontSize: "13px", color: "var(--muted)", marginBottom: "8px" }}>Monthly Budget (USD)</label>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <DollarSign size={18} style={{ color: "var(--faint)" }} />
            <input
              type="number"
              min="0"
              step="5"
            ...
            />
            <span style={{ fontSize: "12px", color: "var(--faint)" }}>API calls will stop if this limit is reached</span>
          </div>
        </div>
      </div>
      ...
      <div style={cardStyle}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "16px" }}>
          <Key size={18} style={{ color: "var(--neg-text)" }} />
        ...
        <div style={{ marginBottom: "16px" }}>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            style={{ display: "inline-flex", alignItems: "center", gap: "4px", fontSize: "12px", color: "var(--neg-text)", marginTop: "8px", textDecoration: "none" }}
          >
        ...
        <div style={{
          ...
          <AlertCircle size={18} style={{ color: "var(--accent-text)", flexShrink: 0, marginTop: "2px" }} />
          <div style={{ fontSize: "12px", color: "var(--blue-pale)" }}>
        ...
        <div style={cardStyle}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              <Activity size={18} style={{ color: "var(--pos)" }} />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "12px", marginBottom: "20px" }}>
            <div style={statBox}>
              <div style={{ fontSize: "11px", color: "var(--muted)", marginBottom: "6px" }}>Total Cost</div>
              <div style={{ fontSize: "20px", fontWeight: "700", fontFamily: "'Barlow Condensed', sans-serif" }}>{formatCost(usageStats.totalCost)}</div>
            </div>
            <div style={statBox}>
              <div style={{ fontSize: "11px", color: "var(--muted)", marginBottom: "6px" }}>Remaining</div>
              <div style={{ fontSize: "20px", fontWeight: "700", color: "var(--pos)", fontFamily: "'Barlow Condensed', sans-serif" }}>{formatCost(usageStats.remainingBudget)}</div>
            </div>
            <div style={statBox}>
              <div style={{ fontSize: "11px", color: "var(--muted)", marginBottom: "6px" }}>Requests</div>
              <div style={{ fontSize: "20px", fontWeight: "700", fontFamily: "'Barlow Condensed', sans-serif" }}>{usageStats.requestCount}</div>
            </div>
            <div style={statBox}>
              <div style={{ fontSize: "11px", color: "var(--muted)", marginBottom: "6px" }}>Tokens Used</div>
          ...
          <div style={{ marginBottom: "20px" }}>
            ...
            <div style={{ height: "8px", background: "var(--outline-variant)", borderRadius: "4px", overflow: "hidden" }}>
              <div style={{
                ...
                background: usageStats.budgetUsedPercent > 90 ? "var(--neg)"
                  : usageStats.budgetUsedPercent > 70 ? "var(--warn)" : "var(--pos)",
                borderRadius: "4px",
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "6px", maxHeight: "240px", overflowY: "auto" }}>
                {usageStats.requests.slice().reverse().slice(0, 10).map((request, index) => (
                  <div key={index} style={{
                    ...
                    padding: "10px 14px", background: "var(--input-bg)", borderRadius: "8px", fontSize: "12px",
                  }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                      <span style={{ fontWeight: "600" }}>{request.feature}</span>
                      <span style={{ color: "var(--faint)" }}>{formatTokens(request.inputTokens + request.outputTokens)} tokens</span>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                      <span style={{ fontWeight: "600" }}>{formatCost(request.cost)}</span>
                      <span style={{ color: "var(--faint)" }}>{new Date(request.timestamp).toLocaleDateString()}</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "16px" }}>
        <button
          onClick={handleSave}
          style={{
            padding: "10px 28px", background: "var(--blue)", border: "none",
      ...
      <div style={{
        ...
        <h4 style={{ fontWeight: "600", color: "var(--text)", marginBottom: "10px", marginTop: 0 }}>Getting Started</h4>
        <ol style={{ margin: 0, paddingLeft: "20px", lineHeight: "2" }}>
          <li>Get your Claude API key from <a href="https://console.anthropic.com/" target="_blank" rel="noopener noreferrer" style={{ color: "var(--accent-text)", textDecoration: "none" }}>Anthropic Console</a></li>
          <li>Get your YouTube API key from <a href="https://console.cloud.google.com/" target="_blank" rel="noopener noreferrer" style={{ color: "var(--neg-text)", textDecoration: "none" }}>Google Cloud Console</a></li>
